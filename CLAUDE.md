...
- **Nine gates exist; none run automatically — there is no CI.** `types:check` regenerates the collection, generates Next types, then type-checks. `pnpm test` runs the script suites. `build` runs `versioned-docs-check` first. Everything else must be invoked by hand.
