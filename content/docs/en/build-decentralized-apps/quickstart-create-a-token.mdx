...
Deploying an **ERC-20** token on <a data-quicklook-from="arbitrum-one">Arbitrum One</a> is fully permissionless and uses the same tooling and workflows as Ethereum.

You can deploy directly to Arbitrum One using Foundry, and optionally configure [bridging](/docs/arbitrum-essentials/bridging/overview), liquidity, and supporting <a data-quicklook-from="smart-contract">smart contract</a> infrastructure as part of your token creation.
...
A list of faucets is available on the [Chain Info page](/docs/chain-info#faucet-list).

Faucets may require certain actions, for example, bridging, use the official Arbitrum Bridge if the faucet requires it.
...
A list of chain IDs is available on the [Chain Info page](/docs/chain-info#arbitrum-public-rpc-endpoints).
...
- Ensure your wallet has enough **ETH** for gas on the target network. Arbitrum fees are low, but mainnet deployments still cost real **ETH**. See [How to estimate gas](/docs/arbitrum-essentials/how-to-estimate-gas) to size your deployment funds.
...
- Use the [Arbitrum Token Bridge](https://bridge.arbitrum.io) to create an L2 counterpart. For a programmatic setup, see the [token bridging guides](/docs/arbitrum-essentials/bridging/overview).
