...
The Rollup chain consists of assertions, which serve as checkpoints summarizing multiple child chain blocks. Assertions are what give Arbitrum its hard finality guarantee — for the broader soft- vs. hard-finality picture, see [Finality](/docs/how-arbitrum-works/deep-dives/sequencer#finality).
...
Validators validate transactions by computing the next <a data-quicklook-from="chain-state">chain state</a> using the chain's [STF](/docs/how-arbitrum-works/deep-dives/stf-gentle-intro), whereas proposers can also assert and challenge the chain state on the parent chain.
