...
The Rollup chain consists of assertions, which serve as checkpoints summarizing multiple child chain blocks. Assertions are what give Arbitrum its <a data-quicklook-from="hard-finality">hard finality</a> guarantee — for the broader soft- vs. hard-finality picture, see [Finality](/docs/how-arbitrum-works/deep-dives/finality).
...
For more details on how the Rollup chain works under <a data-quicklook-from="bold">BoLD</a>, the [gentle introduction](/docs/how-arbitrum-works/bold/gentle-introduction) provides an overview that touches on the Rollup chain.

<VanillaAdmonition type="note" title="Validators and <a data-quicklook-from=&quot;proposer&quot;>proposers</a> serve different roles">

Validators validate transactions by computing the next <a data-quicklook-from="chain-state">chain state</a> using the chain's [STF](/docs/how-arbitrum-works/deep-dives/stf), whereas proposers can also assert and challenge the chain state on the <a data-quicklook-from="parent-chain">parent chain</a>.
...
<VanillaAdmonition type="note" title="Arbitrum Nitro chains accept **ETH** as the only collateral for staking.">
