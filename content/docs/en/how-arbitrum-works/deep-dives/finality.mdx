---
title: Finality
description: 'Learn the fundamentals of finality on Arbitrum.'
...
2. **<a data-quicklook-from="hard-finality">Hard finality</a>**: Occurs when transactions are included in <a data-quicklook-from="batch">batches</a> posted to and finalized on the <a data-quicklook-from="parent-chain">parent chain</a>, providing strong security assurances. Final settlement on the parent chain happens via <a data-quicklook-from="assertion">assertions</a> confirmed by the <a data-quicklook-from="arbitrum-rollup-protocol">Rollup protocol</a>.

This section explores the concepts of soft and hard finality, their implications, trust considerations, and guidance for using them effectively within the Arbitrum network. For the block-tag-level detail—confirmation levels, reorg depth, and indexer guidance—see [Finality and chain reorganizations](/docs/how-arbitrum-works/reference/finality-and-reorgs).

## Authority and finality

- **Exclusive access**: Only authorized [batch posters](/docs/how-arbitrum-works/deep-dives/batchposter) can call these batch-submission methods on the <a data-quicklook-from="sequencer-inbox">Sequencer Inbox</a> contract (access is gated by the contract's `isBatchPoster` mapping, a role distinct from the sequencer, though the two are typically operated by the same party). This exclusivity ensures that no other party can include messages directly.
...
| Parent-chain data finality (batch posted and finalized on <a data-quicklook-from="L1">L1</a>) | Transaction data is durably recorded and inherits the parent chain's consensus security | ~2 epochs, roughly 12–15 minutes (the batch poster defaults to referencing the L1 `safe` block)          |
| Full settlement (assertion confirmed)                         | <a data-quicklook-from="L2">L2</a> state is finalized and L2→L1 withdrawals can be executed                             | A bounded <a data-quicklook-from="challenge-period">challenge period</a> under <a data-quicklook-from="bold">BoLD</a>, on the order of days on mainnet |

## Soft finality

Soft finality refers to the preliminary confirmation of transactions based on the [Sequencer’s](/docs/how-arbitrum-works/deep-dives/sequencer) real-time feed. Key characteristics include:
...
- **High latency**: Achieving hard finality takes longer because the parent chain must process and finalize batches. The <a data-quicklook-from="challenge-protocol">challenge protocol</a> that backs this guarantee is described in BoLD.
