...
The parent chain component is the product of the <a data-quicklook-from="transaction">transaction</a>'s estimated contribution to its <a data-quicklook-from="batch">batch</a>'s size—computed using [Brotli](/docs/how-arbitrum-works/deep-dives/sequencer#compression) on the transaction alone—and the child chain's view of the parent chain data price. This value dynamically adjusts over time to ensure the batch poster is ultimately fairly compensated.

The child chain component consists of the traditional fees <a data-quicklook-from="geth">Geth</a> would pay to bonders in a vanilla parent chain, such as the computation and storage charges that apply to the [<a data-quicklook-from="state-transition-function">State Transition Function</a> (STF)](/docs/how-arbitrum-works/deep-dives/stf-gentle-intro). [<a data-quicklook-from="arbos">ArbOS</a>](/docs/how-arbitrum-works/deep-dives/arbos) charges additional fees for executing its child-chain-specific [precompiles](/docs/arbitrum-essentials/precompiles/overview), whose fees are dynamically priced based on the resources used during execution.
...
- **Compression complexity**: The data posted to the parent chain is compressed using a general-purpose compression algorithm ([Brotli](/docs/how-arbitrum-works/deep-dives/sequencer#compression)). The effectiveness of compression depends on shared patterns among transactions in a batch.
...
For an operational overview of how the Sequencer applies these levels, see [Compression](/docs/how-arbitrum-works/deep-dives/sequencer#compression). Dynamic adjustments of compression levels are based on backlog size ($B$):
