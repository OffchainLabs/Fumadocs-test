...
<a data-quicklook-from="arbitrum">Arbitrum</a> uses gas to track the execution cost on an Arbitrum Nitro chain. It works the same as Ethereum gas, in the sense that every EVM instruction costs the same amount of gas as it would on Ethereum.
...
The parent chain component is the product of the <a data-quicklook-from="transaction">transaction</a>'s estimated contribution to its <a data-quicklook-from="batch">batch</a>'s size—computed using [Brotli](/docs/how-arbitrum-works/deep-dives/batchposter#3-messages-become-a-compressed-batch) on the transaction alone—and the child chain's view of the parent chain data price. This value dynamically adjusts over time to ensure the <a data-quicklook-from="batch-poster">batch poster</a> is ultimately fairly compensated.

The child chain component consists of the traditional fees <a data-quicklook-from="geth">Geth</a> would pay to bonders in a vanilla parent chain, such as the computation and storage charges that apply to the [<a data-quicklook-from="state-transition-function">State Transition Function</a> (STF)](/docs/how-arbitrum-works/deep-dives/stf). [<a data-quicklook-from="arbos">ArbOS</a>](/docs/how-arbitrum-works/deep-dives/arbos) charges additional fees for executing its child-chain-specific [precompiles](/docs/arbitrum-essentials/precompiles/overview), whose fees are dynamically priced based on the resources used during execution.
...
Batch posting costs reflect the actual cost a batch poster pays to post batch data on the parent chain. Whenever a batch is posted, the parent chain contract that records it will send a special "batch posting report" message to the child chain ArbOS, reporting who paid for the batch and the parent chain basefee at the time. This message is placed in the chain's [<a data-quicklook-from="delayed-inbox">delayed inbox</a>](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#transacting-via-the-delayed-inbox) so that it will be delivered to the child chain ArbOS after some delay.
...
The second type of parent chain cost is an optional (per chain) per-unit reward for handling transaction calldata. In general, the reward might be paid to the [<a data-quicklook-from="sequencer">Sequencer</a>](/docs/how-arbitrum-works/deep-dives/sequencer), or to members of the Data Availability Committee in an [AnyTrust chain](/docs/how-arbitrum-works/deep-dives/anytrust-protocol), or to anyone else who incurs per-calldata-byte costs on behalf of the chain. The reward is a fixed number of wei per data unit, and is paid to a single address.
...
The child chain gas price on a given <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> has a set floor, which can be queried via [`ArbGasInfo`](/docs/arbitrum-essentials/precompiles/reference#arbgasinfo)'s `getMinimumGasPrice` method.
...
The security of Arbitrum Nitro chains depends on the assumption that when one [<a data-quicklook-from="validator">validator</a>](/docs/how-arbitrum-works/deep-dives/assertions) creates an [assertion](/docs/how-arbitrum-works/deep-dives/assertions), other validators check it and respond with a correct assertion or a [<a data-quicklook-from="challenge">challenge</a>](/docs/how-arbitrum-works/bold/gentle-introduction) if it's wrong.
