...
ArbOS is the child-EVM virtual machine monitor (VMM) for <a data-quicklook-from="arbitrum-nitro">Arbitrum Nitro</a>: a trusted system component that lives inside the [State Transition Function (STF)](/docs/how-arbitrum-works/deep-dives/stf) and provides the execution environment for the chain. Unlike a conventional operating system, ArbOS is not a process running beside the chain—it's ordinary Go code compiled into the STF, so every node (and fraud prover) can replay it deterministically.
...
2. **Block production**: Turning [Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer)-delivered messages into child-chain blocks with correct state updates.
3. **Cross-chain messaging**: Enabling cross-chain communication between parent and child chains for deposits, withdrawals, and <a data-quicklook-from="retryable-ticket">retryable tickets</a>.
...
- `sendMerkle` is the accumulator that records every L2 → L1 message; its root is what the parent-chain <a data-quicklook-from="outbox">Outbox</a> verifies against when a withdrawal is finalized.
...
- Parent-chain data is tracked by `l1PricingState`, which records what the [batch poster](/docs/how-arbitrum-works/deep-dives/batchposter) spent posting compressed data to Ethereum and reimburses the batch poster from user fees.
