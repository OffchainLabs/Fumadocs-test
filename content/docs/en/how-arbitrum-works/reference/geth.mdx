...
[Arbitrum Nitro](/docs/how-arbitrum-works/inside-arbitrum-nitro) makes minimal modifications to <a data-quicklook-from="geth">Geth</a> to avoid violating its assumptions. This section will explore the relationship between Geth and [<a data-quicklook-from="arbos">ArbOS</a>](/docs/how-arbitrum-works/deep-dives/arbos), which consists of a series of hooks, interface implementations, and strategic re-appropriations of Geth’s basic types.

We store ArbOS's state at an address within a Geth `statedb` (state database). In doing so, ArbOS inherits the `statedb`'s statefulness and lifetime properties. For example, a <a data-quicklook-from="transaction">transaction's</a> direct state changes to ArbOS would get discarded upon a revert.
...
If the transaction is `ArbitrumDepositTx`, ArbOS adds balance to the destination account. This approach is safe because the <a data-quicklook-from="parent-chain">parent chain</a> [<a data-quicklook-from="bridge">bridge</a>](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging) submits such a transaction only after collecting the same amount of funds on the parent chain.
...
| [`ArbitrumRetryTx`](#arbitrumretrytx)                     | A [<a data-quicklook-from="retryable-redeem">retryable redeem</a>](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#retryable-tickets) attempt | [`EndTxHook`](#endtxhook)     | Child chain |
...
The root hash and leaf count of ArbOS's [send Merkle accumulator](https://github.com/OffchainLabs/nitro/blob/8e786ec6d1ac3862be85e0c9b5ac79cbd883791c/arbos/merkleAccumulator/merkleAccumulator.go#L13) are stored in each child chain block's `MixDigest` and `ExtraData` fields to aid with [<a data-quicklook-from="outbox">outbox</a>](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging) proof construction. The yellow paper specifies that the `ExtraData` field may be no larger than 32 bytes, so we use the first 8 bytes of the `MixDigest`, which has no meaning in a system without miners/bonders, to store the send count.
