...
Let's [follow a user's transaction](/docs/how-arbitrum-works/deep-dives/transaction-lifecycle) through this process.

First, the user creates a transaction, signs it with their <a data-quicklook-from="wallet">wallet</a>, and sends it to the Nitro chain's [Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer). The Sequencer's job, as its name implies, is to take the arriving transactions, put them into an ordered sequence, and publish that sequence.

Once the transactions are sequenced, they are run through the [_state transition function_](/docs/how-arbitrum-works/deep-dives/stf), one by one. The state transition function (STF) takes as input the current state of the chain (account balances, contract code, and so on) along with the next transaction. It updates the state and sometimes emits a new Layer 2 block on the Nitro chain.
...
The second key design idea in Nitro is "Geth at the core." Here, "Geth" refers to [go-ethereum](/docs/how-arbitrum-works/reference/geth), the most common node software for Ethereum. As its name would suggest, go-ethereum is written in the Go programming language, as is almost all of Nitro.
...
The key security property of the rollup protocol is that any one honest validator can force the correct execution of the chain to be confirmed. This means that executing an Arbitrum chain is as trustless as executing an Ethereum chain. You, and you alone (or someone you hire), can ensure your transactions are processed correctly. And this is true no matter how many malicious people are trying to stop you. Under BoLD, this guarantee comes with a [_bounded delay_](/docs/how-arbitrum-works/bold/bold-technical-deep-dive): a single honest party can force <a data-quicklook-from="confirmation">confirmation</a> of the correct outcome with a known, fixed amount of time, regardless of how many adversaries participate or how much they place as bond.

### The Rollup chain

The rollup protocol tracks a chain of claims about the L2 state—we'll call these [assertions](/docs/how-arbitrum-works/deep-dives/assertions). They're not the same as Layer 1 Ethereum blocks, and also not the same as Layer 2 (Nitro) blocks. You can think of these assertions as forming a tree of claims about the chain's history, which the <a data-quicklook-from="arbitrum-rollup-protocol">Arbitrum Rollup Protocol</a> manages and oversees.
...
At any given time, some validators will be bonders, and some will not. To propose an assertion, a validator must put up a [bond held by the Arbitrum Layer 1 contracts](/docs/how-arbitrum-works/bold/bold-economics-of-disputes), which will be confiscated if the assertion is proven false. The bond is denominated in an **ERC-20** token configured for the chain—on Arbitrum One, this is **WETH**—so, in practice, stakers post **ETH**-denominated value.
...
An L1 contract can [submit an L2 transaction](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging), just like a user would, by calling the Nitro chain's inbox contract on Ethereum. This L2 transaction will run later, producing results that will not be available to the L1 caller. The transaction will execute at L2, but the L1 caller won't be able to see any results from the L2 transaction.
...
This would introduce a serious problem for certain types of L1-to-L2 interactions. Consider a transaction that [deposits a token on L1](/docs/how-arbitrum-works/deep-dives/token-bridging) and makes it available at some address on L2. If the L1 side succeeds, but the L2 side reverts, you've just sent some tokens to the inbox contract that are unrecoverable on either L2 or L1. Not good.
...
Calls from L2-to-L1 operate similarly, using a ticket-based system. An L2 contract can call a method of the precompiled ArbSys contract to [send a transaction to L1](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging). When the execution of the L2 transaction containing the submission is confirmed at L1 (some days later), a ticket is created in the L1 <a data-quicklook-from="outbox">outbox</a> contract. That ticket can be triggered by anyone who calls a certain L1 outbox method and submits the `ticketID`. The ticket is only marked as redeemed if the L1 transaction does not revert.
...
User transactions pay [fees to cover the chain's operating costs](/docs/how-arbitrum-works/deep-dives/gas-and-fees). These fees are assessed and collected by ArbOS at the L2 level. They are denominated in **ETH**.
...
[AnyTrust](/docs/how-arbitrum-works/deep-dives/anytrust-protocol) is a variant of the Arbitrum Nitro technology that reduces costs by accepting a weaker trust assumption.
...
- [State Transition Function: Gentle Introduction](/docs/how-arbitrum-works/deep-dives/stf): Foundational concepts of the STF
