...
Timeboost is a _transaction ordering policy_. It's a set of rules that the [sequencer](/docs/how-arbitrum-works/deep-dives/sequencer) of an Arbitrum chain is trusted to follow when ordering transactions submitted by users. In the near future, multiple sequencers will be able to enforce those rules with decentralized Timeboost.

For Arbitrum chains, the sequencer’s sole job is to take arriving, valid transactions from users, place them into an order dictated by the transaction ordering policy, and then publish the final sequence to a [real-time feed](/docs/how-arbitrum-works/deep-dives/sequencer#4-persistence-and-broadcast) and in compressed batches to the chain’s data availability layer. Before Timeboost, the transaction ordering policy was FCFS, and Timeboost is a modified FCFS ordering policy.
...
<ImageZoom src="/img/timeboost-centralized-timeboost-express-lane-workflow.jpg" alt="using the express lane" className="img-900px" />
...
<VanillaAdmonition type="info" title="Bid behavior">
...
<VanillaAdmonition type="info" title="Additional FAQs">

For frequently asked questions refer to the [Timeboost FAQ](/docs/how-arbitrum-works/timeboost/timeboost-faq).
