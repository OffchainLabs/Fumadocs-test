...
description: 'Understand how Arbitrum handles block gas limits, block numbers, and transaction timing differently from Ethereum. Learn about parent chain gas components and block.number behavior on Arbitrum.'
user_story: 'As a developer, I want to understand how block numbers, gas limits, and timing work on Arbitrum so I can write smart contracts that account for these differences.'
content_type: 'concept'
author: dzgoldman, jose-franco
sme: jose-franco
---

<VanillaAdmonition type="info" title="block number vs `block.number`">
...
As with Ethereum, Arbitrum clients submit transactions, and the system executes them later. In Arbitrum, clients submit transactions by posting messages to the Ethereum chain, either [through the Sequencer](/docs/how-arbitrum-works/deep-dives/sequencer) or via the chain's [Delayed Inbox](/docs/how-arbitrum-works/deep-dives/sequencer).
...
Additionally, since the calldata/blobs of Arbitrum transactions (or the DAC certificate on <Term id="arbitrum-anytrust-chain">AnyTrust</Term>chains) is posted to Ethereum, the gas paid when executing them includes a component for the parent chain to cover the costs of the batch poster.
...
The Arbitrum block number can also be retrieved within an Arbitrum contract via the [ArbSys](/docs/arbitrum-essentials/precompiles/reference#arbsys) precompile:
