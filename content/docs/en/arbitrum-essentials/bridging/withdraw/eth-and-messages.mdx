...
description: 'Step-by-step guide to programmatically withdraw ETH and send messages from an Arbitrum child chain to Ethereum using ArbSys and the Outbox contract.'
user_story: 'As a developer, I want to programmatically withdraw assets and messages from Arbitrum to Ethereum so I can build cross-chain functionality in my dApp.'
content_type: 'how-to'
author: gblanchemain
sme: gblanchemain
---

This guide explains how to programmatically send messages and withdraw assets from an <a data-quicklook-from="arbitrum">Arbitrum</a> <a data-quicklook-from="child-chain">child chain</a> to a <a data-quicklook-from="parent-chain">parent chain</a> (such as Ethereum). For conceptual information about the messaging protocol, see [Child-to-parent chain messaging](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging).

## Prerequisites

- A child chain <a data-quicklook-from="wallet">wallet</a> with funds
...
2. **Wait for finalization**: The message enters a 6.4-day <a data-quicklook-from="challenge-period">Challenge Period</a>
...
Returns a unique identifier for the message, used to track its status and construct the <a data-quicklook-from="outbox">outbox</a> proof for execution.
...
- **`size`**: The <a data-quicklook-from="batch">batch</a> number containing your message
...
<VanillaAdmonition type="note" title="`NodeInterface` is a &quot;virtual&quot; contract accessible at `0x00000000000000000000000000000000000000C8`. It isn't a true <a data-quicklook-from=&quot;precompile&quot;>precompile</a> but provides Arbitrum-specific data without requiring a custom RPC.">
...
2. **Challenge period**: Wait 6.4 days for the <a data-quicklook-from="assertion">assertion</a> to finalize
...
<ImageZoom src="/img/apps-withdrawing-ether.svg" alt="Process that funds follow during a withdrawal operation" className="img-800px" />

## Withdrawing **ERC-20** tokens

For **ERC-20** token withdrawals, see the dedicated [Withdraw tokens guide](/docs/arbitrum-essentials/bridging/withdraw/tokens), which provides detailed instructions for using Arbitrum's canonical token <a data-quicklook-from="bridge">bridge</a>.
...
- For protocol-level details, see [Child to parent chain messaging](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging)
- For token bridging concepts, see [Token bridging overview](/docs/how-arbitrum-works/deep-dives/token-bridging)
