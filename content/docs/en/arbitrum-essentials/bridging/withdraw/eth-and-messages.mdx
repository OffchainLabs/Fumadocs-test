...
This guide explains how to programmatically send messages and withdraw assets from an <Term id="arbitrum">Arbitrum</Term> <Term id="child-chain">child chain</Term> to a <Term id="parent-chain">parent chain</Term> (such as Ethereum). For conceptual information about the messaging protocol, see [Child-to-parent chain messaging](/docs/how-arbitrum-works/deep-dives/l2-to-l1-messaging).

## Prerequisites

- A child chain <Term id="wallet">wallet</Term> with funds
...
2. **Wait for finalization**: The message enters a 6.4-day <Term id="challenge-period">Challenge Period</Term>
...
Returns a unique identifier for the message, used to track its status and construct the <Term id="outbox">outbox</Term> proof for execution.
...
- **`size`**: The <Term id="batch">batch</Term> number containing your message
...
<VanillaAdmonition type="note" title="`NodeInterface` is a &quot;virtual&quot; contract accessible at `0x00000000000000000000000000000000000000C8`. It isn't a true precompile but provides Arbitrum-specific data without requiring a custom RPC.">
...
2. **Challenge period**: Wait 6.4 days for the <Term id="assertion">assertion</Term> to finalize
...
For **ERC-20** token withdrawals, see the dedicated [Withdraw tokens guide](/docs/arbitrum-essentials/bridging/withdraw/tokens), which provides detailed instructions for using Arbitrum's canonical token <Term id="bridge">bridge</Term>.
