...
Debugging smart contracts can be challenging, especially when dealing with complex transactions. The `cargo-stylus` crate simplifies the debugging process by providing multiple tools for replaying, tracing, and interactively debugging <Term id="stylus">Stylus</Term> transactions. These tools enable you to set breakpoints, inspect state changes, and trace execution step by step.
...
3. **Interactive debugging** (`cargo stylus replay`): Replay and debug <Term id="transaction">Transaction</Term> execution using GDB, LLDB, or StylusDB. Set breakpoints, inspect variables, and step through code line by line.
...
- **[Arbitrum RPC provider](#rpc-endpoint-compatibility)** with tracing endpoints enabled or a [local Stylus dev node](/docs/run-arbitrum-node/run-nitro-dev-node)
...
This produces a JSON trace showing functions called and [ink](/docs/stylus/concepts/gas-metering#ink-and-gas) consumption:
...
Both tracers are available on local nodes, but `stylusTracer` is more efficient. See the [list of RPC providers](/docs/for-devs/dev-tools-and-resources/chain-info#third-party-rpc-providers) for tracing support information.

## Limitations

<VanillaAdmonition type="warning" title="Cannot debug deployment or activation transactions">
