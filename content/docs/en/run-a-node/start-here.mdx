...
<VanillaAdmonition type="info" title="If you’re interested in accessing an Arbitrum chain but don’t want to set up your own node, see our [Node Providers](/docs/arbitrum-essentials/reference/node-providers) to get RPC access to fully managed nodes hosted by a third-party provider.">
...
<VanillaAdmonition type="warning" title="Although there are beta and release candidate versions of the Arbitrum Nitro software, use only the release version when running your node. Running beta or RC versions is not supported and might lead to unexpected behaviors and/or database corruption.">
