...
If you're looking to run a node with a different role — <Term id="sequencer">sequencer</Term>, <Term id="batch-poster">batch poster</Term>, <Term id="validator">validator</Term>, archive node, or feed relay — see [How to assign roles to a Nitro node](/docs/run-a-node/assign-node-roles) for the flags that define each role.
