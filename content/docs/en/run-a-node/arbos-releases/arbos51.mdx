...
- If the Arbitrum chain posts blobs to a Fusaka-enabled <a data-quicklook-from="parent-chain">Parent chain</a>, ensure that the consensus layer client is configured correctly as per the [Fusaka upgrade notice](/docs/notices/fusaka-upgrade-notice).
...
As [disclosed previously](https://forum.arbitrum.foundation/t/disclosure-of-support-for-eip-2537-on-arbitrum-one-and-nova/29720), the precompiled contracts for performing various operations over the BLS12-381 elliptic curve, including <a data-quicklook-from="bls-signature">BLS Signature</a> verification, were added but not properly enabled in [ArbOS 40 Callisto](/docs/run-a-node/arbos-releases/arbos40) as originally expected. ArbOS 51 Dia will now enable `EIP-2537`.
