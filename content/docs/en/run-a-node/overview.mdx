...
description: Learn more about what types of Arbitrum nodes are available to run.
...
There is no protocol-level incentive to run an Arbitum full node. If you’re interested in accessing an Arbitrum chain but don’t want to set up a node locally, see our [RPC endpoints and providers](/docs/arbitrum-essentials/reference/node-providers) to get RPC access to fully managed nodes hosted by a third-party provider.

</VanillaAdmonition>

<VanillaAdmonition type="warning" title="API security disclaimer">
...
- You can use [ third party node providers ](/docs/arbitrum-essentials/reference/node-providers) to get RPC access to fully-managed nodes
...
Beyond full and archive nodes, a Nitro node can take on other roles — <a data-quicklook-from="sequencer">sequencer</a>, <a data-quicklook-from="batch-poster">batch poster</a>, <a data-quicklook-from="validator">validator</a>, or feed relay — purely through configuration. For the flags that define each role and how to convert a node between roles, see [How to assign roles to a Nitro node](/docs/run-a-node/assign-node-roles).
...
For detailed instructions, read [how to run an Arbitrum full node](/docs/run-a-node/run-full-node).
...
For detailed instructions, read [how to run an Arbitrum archive node](/docs/run-a-node/more-types/run-archive-node).
...
For detailed instructions, read [how to run an Arbitrum classic node](/docs/run-a-node/more-types/run-classic-node).
...
For detailed instructions, read [how to run an Arbitrum feed relay](/docs/run-a-node/run-feed-relay).
