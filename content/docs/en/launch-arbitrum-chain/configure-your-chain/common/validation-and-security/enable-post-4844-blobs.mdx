...
This guide explains how to configure your <a data-quicklook-from="arbitrum">Arbitrum</a> node to post [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) blob transactions to the <a data-quicklook-from="parent-chain">parent chain</a>, which can significantly reduce data availability costs.
...
- Your <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> must be running in **Rollup mode**
...
Your <a data-quicklook-from="arbos">ArbOS</a> version must be **20 or higher**. To check your current version:

#### Method 1: Smart contract call

Call the `arbOSVersion()` function on the ArbSys <a data-quicklook-from="precompile">precompile</a> contract:

- Contract address: `0x0000000000000000000000000000000000000064`
- Function: `arbOSVersion()` returns `uint256`
- You can call this using any Ethereum <a data-quicklook-from="client">client</a> or block explorer on your Arbitrum chain
...
To enable blob <a data-quicklook-from="transaction">transaction</a> posting, add the following configuration to your <a data-quicklook-from="batch-poster">Batch Poster</a> node:
