...
<VanillaAdmonition type="note" title="This documentation is for production sequencer deployments. If you want to set up a sequencer for testing or on a testnet, please refer to [How to run a testnet sequencer node](/docs/run-arbitrum-node/sequencer/run-sequencer-node.mdx).">
...
You can check [run a feed relay](/docs/run-arbitrum-node/run-feed-relay.mdx) to see how to set up a relay node.
...
If you run into any issues, visit the [node-running troubleshooting guide](/docs/run-arbitrum-node/troubleshooting.mdx).
