...
<VanillaAdmonition type="info" title="The Arbitrum chain SDK">
...
Additionally, Arbitrum chains can be configured to use **ETH** or any standard **ERC-20** token as the gas token. To understand the implications of using a custom gas token, see [Configure a custom gas token](/docs/launch-arbitrum-chain/configure-your-chain/common/gas/use-a-custom-gas-token-rollup.mdx).

## 2. Deploy your chain

After selecting a chain type, follow [this guide](/docs/launch-arbitrum-chain/deploy-an-arbitrum-chain/deploying-an-arbitrum-chain.mdx) to deploy your chain using the Chain SDK.

## 3. Configure your Arbitrum chain's node

Once the chain is deployed, you'll need to generate the configuration to run its node. To learn how, visit [Configure your Arbitrum chain's node](/docs/launch-arbitrum-chain/how-tos/arbitrum-chain-sdk-preparing-node-config.mdx).

## 4. Deploy your Arbitrum chain's token bridge

Your Arbitrum chain's token bridge contracts allow **ERC-20** tokens to move between your Arbitrum chain and its underlying parent chain. Read [Deploy your Arbitrum chain's token bridge](/docs/launch-arbitrum-chain/deploy-an-arbitrum-chain/deploying-token-bridge.mdx) to learn how to set up your bridge.
