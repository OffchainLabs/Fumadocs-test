...
This document is provided for informational purposes only and does not constitute an end-to-end guide for executing the migration. <a data-quicklook-from="offchain-labs">Offchain Labs</a> doesn’t have special knowledge or a private “fast path.” This document provides the extent of our guidance. Your current <a data-quicklook-from="raas">RaaS</a> provider owns and executes the migration. We can provide [introductions to RaaSes](./third-party-integrations/third-party-providers#rollup-as-a-service-raas-providers).
...
- **Lower fees**: <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> Rollup or AnyTrust for low <a data-quicklook-from="transaction">transaction</a> fees.
- **Various customization options**: Custom gas token, flexible DA, <a data-quicklook-from="timeboost">Timeboost</a>, <a data-quicklook-from="bold">BoLD</a>, etc.
...
- **Ecosystem migration (simpler, faster)**: Launch a fresh Arbitrum chain. Users/app <a data-quicklook-from="bridge">Bridge</a> funds and redeploy contracts. This approach is quicker and less risky, but it puts a burden on apps and users.
...
Zero knowledge (zk) pre-compiles don’t carry over. Port logic to Solidity or <a data-quicklook-from="stylus">Stylus</a> or remove/replace functionality. Please check with your RaaS regarding this matter.
...
- **<a data-quicklook-from="chain-state">Chain state</a> migration:** A more complex migration where the chain state gets transplanted into the new Rollup stack/contract system, but the ecosystem and user experience may stay relatively the same.
...
1. Freeze legacy chain writes and make RPC read-only:
   1. Stop the legacy chain’s proposer/<a data-quicklook-from="sequencer">Sequencer</a> so no new blocks or transactions get created after the chosen upgrade block.
...
2. Ensure that you can export required state from your EVM-compatible <a data-quicklook-from="client">client</a> at the upgrade block.
...
- [ ] Testnet deployment: Sequencer, <a data-quicklook-from="batch-poster">Batch Poster</a>, <a data-quicklook-from="bonder">Bonder</a>, DA config.
...
- [ ] Start batch poster/bonder and verify assertions on <a data-quicklook-from="l1">L1</a>.
