...
In this how-to, you'll learn how to configure the DAC in your chain. Refer to the [Introduction](/docs/launch-arbitrum-chain/configure-your-chain/common/data-availability/data-availability-committees/get-started.mdx) for the full process of running DA servers and configuring the chain.
...
- [How to deploy a Data Availability Server (DAS)](/docs/launch-arbitrum-chain/configure-your-chain/common/data-availability/data-availability-committees/deploy-das.mdx). This is needed to understand where the data we'll be handling in this guide comes from.
...
You should also make sure that at least one DAS is running as an [archive DAS](/docs/launch-arbitrum-chain/configure-your-chain/common/data-availability/data-availability-committees/deploy-das.mdx#archive-da-servers), otherwise the information will not be available after the expiry time.
...
To configure the batch poster, we'll use the JSON structure we created in Step 1. This will allow the batch poster to send RPC requests to all the DA servers (to store the information of the <a data-quicklook-from="transaction">transactions</a> being included in the next batch), craft the DACert, and store it in the `SequencerInbox`.
