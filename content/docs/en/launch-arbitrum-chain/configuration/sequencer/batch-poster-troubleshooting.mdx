...
This guide covers common operational issues that batch poster operators encounter, with guidance on diagnosing root causes and resolving them. For initial setup, see [Run a batch poster](/docs/run-a-node/run-batch-poster.mdx). For fee-related tuning, see [Batch poster fee tuning](/docs/launch-arbitrum-chain/configuration/sequencer/batch-poster-fee-tuning.mdx).

## Batch poster balance management

The [batch poster](/docs/how-arbitrum-works/deep-dives/batchposter.mdx) account is an Externally Owned Account (EOA) that pays gas fees on the parent chain to submit batch transactions. There is no automatic mechanism to keep this account funded—operators must monitor the balance and replenish it manually or through external automation.
...
The [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring-tools-and-considerations.mdx) guide recommends monitoring the batch poster balance. In practice, this means:
...
- **Check the sequencer's transaction backlog size**: As noted in the [Monitoring tools and considerations](/docs/launch-arbitrum-chain/operate/monitoring-tools-and-considerations.mdx) guide, a large and growing backlog can be a sign of network health issues.
...
2. **Check the parent chain gas prices**: If prices exceed your fee cap, either wait for prices to drop or increase `--node.batch-poster.data-poster.target-price-gwei` (see [Batch poster fee tuning](/docs/launch-arbitrum-chain/configuration/sequencer/batch-poster-fee-tuning.mdx)).
