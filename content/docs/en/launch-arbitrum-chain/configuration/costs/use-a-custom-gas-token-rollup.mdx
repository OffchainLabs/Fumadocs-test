...
When deploying an <Term id="arbitrum-chain">Arbitrum chain</Term> in [Rollup mode](/docs/run-a-node/data-availability#rollup-mode), you can use a custom **ERC-20** token as the native gas token. This token is usable for <Term id="transaction">Transaction</Term> fees on that specific Arbitrum chain and reimbursing the <Term id="batch-poster">Batch Poster</Term> for data posted to Ethereum. An example would be an <Term id="layer-2-l2">L2</Term> Rollup that uses **USDC** as its custom gas token but pays **ETH** to post data to <Term id="layer-1-l1">L1</Term> Ethereum.
