...
<VanillaAdmonition type="info" title="While Native Mint/Burn was introduced in ArbOS 41, we strongly recommend that teams upgrade to [ArbOS 51 Dia](/docs/run-a-node/arbos-releases/arbos51) to enable this feature.">
...
| `go-ethereum` fork | Included in [ArbOS 51 Dia](/docs/run-a-node/arbos-releases/arbos51) (please use the latest <Term id="wasm">WASM</Term> module root) or later |

We suggest the following baseline or values for production chains:

- [ArbOS version 51](/docs/run-a-node/arbos-releases/arbos51) or later.
...
1. Update your chain to [ArbOS 51 Dia](/docs/run-a-node/arbos-releases/arbos51) binaries and deploy the consensus-v51 WASM root.
...
While native mint/burn was technically introduced in ArbOS 41, we strongly recommend upgrading to [ArbOS 51 Dia](/docs/run-a-node/arbos-releases/arbos51) for those enabling this feature.
