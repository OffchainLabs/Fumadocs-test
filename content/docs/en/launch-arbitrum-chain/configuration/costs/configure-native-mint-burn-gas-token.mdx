...
<VanillaAdmonition type="warning" title="Bridge counterparty risk">

While this feature allows an <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a> to use native interop tokens as gas tokens via third-party protocols, it introduces new trust assumptions. In a standard Arbitrum chain, the canonical bridge is the sole authority for minting the chain's gas token, inheriting the security of the <a data-quicklook-from="parent-chain">parent chain</a>.

However, by using a third-party <a data-quicklook-from="bridge">bridge</a> for the chain's gas token, your chain's gas token—and thus its liveness—becomes directly dependent on the security and integrity of the third-party bridge provider.

</VanillaAdmonition>
<VanillaAdmonition type="info" title="While Native Mint/Burn was introduced in <a data-quicklook-from=&quot;arbos&quot;>ArbOS</a> 41, we strongly recommend that teams upgrade to [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) to enable this feature.">

If you want to learn more about the concept of native mint/burn and the impact on your chain, refer to the [Choose native mint/burn page](/docs/launch-arbitrum-chain/features/common/gas-and-fees/choose-native-mint-burn.mdx).
...
| `go-ethereum` fork | Included in [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) (please use the latest <a data-quicklook-from="wasm">WASM</a> module root) or later |

We suggest the following baseline or values for production chains:

- [ArbOS version 51](/docs/run-arbitrum-node/arbos-releases/arbos51) or later.
...
- <a data-quicklook-from="outbox">Outbox</a> sweep bot: run once per hour to prevent stray collateral build-up.
...
<VanillaAdmonition type="info" title="The native token mint/burn feature is currently not available for implementation at chain genesis. The ability to activate it during chain initialization will be part of a future release.">

</VanillaAdmonition>
### Enable the feature post-genesis

Below steps show the flow a <a data-quicklook-from="chain-owner">chain owners</a> can use to allowlist a bridge adapter. The example below uses a Foundry call, but you can also use other tools to interact with the precompiles directly.

1. Update your chain to [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) binaries and deploy the consensus-v51 WASM root.
...
While native mint/burn was technically introduced in ArbOS 41, we strongly recommend upgrading to [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) for those enabling this feature.
...
<VanillaAdmonition type="warning" title="Testing Responsibility">
