...
While this feature allows an <Term id="arbitrum-chain">Arbitrum chain</Term> to use native interop tokens as gas tokens via third-party protocols, it introduces new trust assumptions. In a standard Arbitrum chain, the canonical bridge is the sole authority for minting the chain's gas token, inheriting the security of the <Term id="parent-chain">parent chain</Term>.

However, by using a third-party <Term id="bridge">bridge</Term> for the chain's gas token, your chain's gas token—and thus its liveness—becomes directly dependent on the security and integrity of the third-party bridge provider.

</VanillaAdmonition>
<VanillaAdmonition type="info" title="While Native Mint/Burn was introduced in ArbOS 41, we strongly recommend that teams upgrade to [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) to enable this feature.">
...
| `go-ethereum` fork | Included in [ArbOS 51 Dia](/docs/run-arbitrum-node/arbos-releases/arbos51) (please use the latest <Term id="wasm">WASM</Term> module root) or later |
...
- <Term id="outbox">Outbox</Term> sweep bot: run once per hour to prevent stray collateral build-up.
...
Below steps show the flow a <Term id="chain-owner">chain owners</Term> can use to allowlist a bridge adapter. The example below uses a Foundry call, but you can also use other tools to interact with the precompiles directly.
