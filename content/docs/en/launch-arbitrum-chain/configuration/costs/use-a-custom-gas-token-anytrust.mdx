...
The main requirements for a custom gas token is that it must be an **ERC-20** token, and there _must be some representation_ on the <a data-quicklook-from="parent-chain">parent chain</a>. During chain deployment, the gas token is "natively bridged" and then properly configured as the native gas token on the <a data-quicklook-from="arbitrum-chain">Arbitrum chain</a>.
...
<VanillaAdmonition type="info" title="While certain restrictions are in place today (see above), some of them may be removed in upcoming releases of the custom gas token feature. Please reach out to the <a data-quicklook-from=&quot;offchain-labs&quot;>Offchain Labs</a> team with any questions or requests for additions to the custom gas token feature.">
...
After deploying the chain, you must reset the base fees of the parent chain by calling the following functions in the [`ArbOwner`](/docs/build-decentralized-apps/precompiles/reference.mdx#arbowner) precompile:

- `SetL1PricePerUnit`, setting `pricePerUnit` to `0`
- `SetL1PricingRewardRate`, setting `perUnitReward` to `0`

<VanillaAdmonition type="note" title="These methods use <a data-quicklook-from=&quot;l1&quot;>L1</a> to refer to the parent chain of the Arbitrum chain.">
...
In either case we strongly recommend taking these steps to avoid any issues with chain economics. You can read more about how Nitro manages fees in [Gas and fees](/docs/how-arbitrum-works/deep-dives/gas-and-fees.mdx).
