...
As a <Term id="raas">RaaS</Term> or <Term id="chain-owner">Chain Owner</Term>, if you require internal daily reporting on fees, we have an [RPC script you can leverage for reporting purposes](https://github.com/yahgwai/aep-fee-tracker/).
...
There are two main <Term id="layer-1-l1">L1</Term> fee collector addresses:

1. **L1 Base Fee**: Refunds <Term id="batch-poster">Batch Poster</Term> for <Term id="parent-chain">parent chain</Term> posting costs
2. **L1 Surplus**: Collects profit on <Term id="batch">Batch</Term> posting fees
...
1. Users send transactions on the <Term id="child-chain">child chain</Term>
...
1. <Term id="arbos">ArbOS</Term> consumes batch posting reports
