...
<VanillaAdmonition type="info" title="The instructions provided in [How to run a full node](/docs/run-arbitrum-node/run-full-node.mdx) **will not** work with your Arbitrum chain node. See [Optional parameters (Arbitrum chain)](/docs/run-arbitrum-node/run-full-node.mdx#optional-parameters) for Arbitrum chain-specific CLI flags.">
...
- The <a data-quicklook-from="wasm-module-root">WASM module root</a> (stored in `module-root.txt`) is a 32 byte hash usually expressed in hexadecimal which is a merkelization of `machine.wavm.br`.
