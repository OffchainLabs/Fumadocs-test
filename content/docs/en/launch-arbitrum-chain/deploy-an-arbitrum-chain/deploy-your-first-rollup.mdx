...
| `PARENT_CHAIN_RPC`                  | Optional               | Step 2, 3, 6 | Arbitrum Sepolia RPC URL. If omitted, uses the default public RPC (can be slow or time out). Use a [node provider](/docs/build-decentralized-apps/reference/node-providers.mdx) for reliability. |
...
import { prepareChainConfig, createRollupPrepareDeploymentParamsConfig, createRollup } from '@arbitrum/chain-sdk';
