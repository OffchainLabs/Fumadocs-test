...
A Nitro node's _role_ is not a separate piece of software. Every role in this article runs the same `nitro` binary; what makes a node a <Term id="sequencer">sequencer</Term>, a <Term id="batch-poster">batch poster</Term>, a <Term id="validator">validator</Term>, or a plain RPC node is the set of configuration flags you pass at startup. The exceptions are the feed relay — a small standalone `relay` binary shipped inside the same Docker image — and the optional split-validation setup, which moves block validation into a separate `nitro-val` binary; both are built from the same repository.
...
Flags can be passed on the command line or collected in a JSON configuration file loaded with `--conf.file`. For how flag names, defaults, and the config file relate to each other, see [Nitro configuration system](/docs/run-a-node/nitro/configuration-system). This article covers only the flags that define and support each role; for the complete flag list, see the [CLI flags reference](/docs/run-a-node/nitro/cli-flags-reference).
...
| ------------------------------------ | ------------------------------------------------------------------------------- | --------------------------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------- |
| RPC node (full node)                 | none (the default)                                                              | No wallet, no bond                                        | Many (horizontal scaling)                 | [Run a full node](/docs/run-a-node/run-full-node)                               |
| Archive node                         | `--execution.caching.archive`                                                   | No wallet, no bond                                        | Many (same as RPC nodes)                  | [Run an archive node](/docs/run-a-node/more-types/run-archive-node)             |
| Sequencer                            | `--node.sequencer` + `--execution.sequencer.enable`                             | No wallet, no bond                                        | One per chain (or one coordinator set)    | [Run a sequencer node](/docs/run-a-node/sequencer/run-sequencer-node)           |
| Batch poster                         | `--node.batch-poster.enable`                                                    | Funded parent-chain wallet; no bond                       | One active (Redis lock coordinates a set) | [Run a batch poster](/docs/launch-arbitrum-chain/run-a-node/batch-poster)                 |
| Validator (staker)                   | `--node.staker.strategy` set to an active value                                 | Funded wallet and bond for active strategies only         | One active per wallet                     | [Run a validator](/docs/run-a-node/more-types/run-validator-node)               |
| Validation server (split validation) | `nitro-val` binary + `--node.block-validator.validation-server.url` on the node | No wallet, no bond (all onchain action stays on the node) | Many (one node can use several)           | [Run a split validator node](/docs/launch-arbitrum-chain/run-a-node/split-validator-node) |
| Feed relay                           | `relay` binary + `--node.feed.input.url` + `--chain.id`                         | No wallet, no bond, no parent-chain connection            | Many (stateless fan-out)                  | [Run a feed relay](/docs/run-a-node/run-feed-relay)                                |

Every role except the feed relay and the split-validation server needs a parent-chain connection through `--parent-chain.connection.url`. Nodes whose parent chain is Ethereum <Term id="L1">L1</Term> also need `--parent-chain.blob-client.beacon-url` to read batches posted as EIP-4844 blobs. For well-known chains, setting `--chain.id` or `--chain.name` auto-fills defaults such as `--execution.forwarding-target` and `--node.feed.input.url` from the chain info embedded in the binary (the forwarding target is only auto-filled when the sequencer is disabled).
...
`--node.staker.enable` defaults to `true`, so a default full node is technically a <Term id="watchtower-validator">watchtower validator</Term>: it watches onchain assertions and logs when one disagrees with its locally computed state. With the default watchtower strategy it loads no wallet and posts no bond, so it takes no onchain action. This is why "converting to a validator" is mostly a matter of changing the strategy and providing a wallet and bond, rather than enabling a module. To silence the watchtower entirely, set `--node.staker.enable=false`.

</VanillaAdmonition>

For the full setup, including snapshots, pruning, and Docker details, see [How to run a full node](/docs/run-a-node/run-full-node).
...
Arbitrum One is the only chain with pre-Nitro (Classic) history, so only Arbitrum One archive nodes need `--execution.rpc.classic-redirect` pointed at an [Arbitrum Classic node](/docs/run-a-node/more-types/run-classic-node); every other chain launched directly on Nitro.

For snapshots, hardware sizing, and the full setup, see [How to run an archive node](/docs/run-a-node/more-types/run-archive-node).

## Sequencer

The sequencer orders incoming transactions, produces blocks, and publishes the <Term id="sequencer-feed">sequencer feed</Term>. Enabling it requires two flags that must agree with each other.
...
For the complete sequencer setup and the coordinator design, see [How to run a sequencer node](/docs/run-a-node/sequencer/run-sequencer-node) and [How to run a Sequencer Coordination Manager (SQM)](/docs/run-a-node/sequencer/run-sequencer-coordination-manager).

## Batch poster

The batch poster compresses queued sequencer messages into batches and posts them to the parent chain's <Term id="sequencer-inbox">Sequencer Inbox</Term>. It does not need to be the sequencer; a separate node can post batches from the messages it receives over the feed.
...
For chain-owner configuration, blob posting, and troubleshooting, see [Run a batch poster](/docs/launch-arbitrum-chain/run-a-node/batch-poster).

## Validator (staker)

A validator watches the chain's onchain assertions and, depending on its strategy, participates in <Term id="bold">BoLD</Term> and legacy disputes. The behavior is set by the strategy, not by a separate enable switch.
...
For the strategy comparison table, wallet setup, and BoLD details, see [How to run a validator](/docs/run-a-node/more-types/run-validator-node).

### Split validation

Block validation — re-executing blocks against the chain's <Term id="wasm">WASM</Term> machines — is the CPU-heavy part of validating. By default this work stays inside the node: with block validation on, the default `--node.block-validator.validation-server.url` value of `self-auth` starts an internal validation server reached over the node's own authenticated websocket loopback. Split validation moves that work to a separate machine running the `nitro-val` binary, while the wallet, the bond, and the record of validation progress all stay on the main node.
...
For Docker images, Helm charts, and the full setup, see [Run a split validator node](/docs/launch-arbitrum-chain/run-a-node/split-validator-node) and [Docker images and CLI binaries](/docs/run-a-node/nitro/docker-and-cli-binaries).
...
For setup, Docker commands, and Kubernetes charts, see [How to run a feed relay](/docs/run-a-node/run-feed-relay).
...
For background on how nodes fit together, see the [Nodes overview](/docs/run-a-node/overview) and, for the data flow between the sequencer, feed, and full nodes, [Data availability](/docs/run-a-node/data-availability#how-full-nodes-sync-the-data-from-the-sequencer-feed).
