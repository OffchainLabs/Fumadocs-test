...
<VanillaAdmonition type="note" title="If you are running Ubuntu 22.04, you might get an `Unable to locate package docker-buildx-plugin` error. Try `sudo apt install docker-buildx` instead.">
...
<VanillaAdmonition type="note" title="Replace `~/.zprofile` with `~/.bash_profile` if you use bash instead of zsh).">
...
### Step 5. Configure node [24](https://github.com/nvm-sh/nvm)
...
### Step 8. Configure Go [1.25](https://github.com/moovweb/gvm)
...
<VanillaAdmonition type="note" title="If you use zsh, replace `bash` with `zsh`.">
