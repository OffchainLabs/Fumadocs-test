---
title: 'RPC methods'
description: 'Compare Arbitrum and Ethereum RPC method responses, including additional transaction types, fields, and blocks returned by Arbitrum nodes. Covers eth_getTransactionByHash, eth_getBlockByHash, and other JSON-RPC methods.'
user_story: 'As a developer, I want to know which RPC methods behave differently on Arbitrum so I can correctly parse responses in my dApp.'
...
In addition to the [three transaction types](https://ethereum.org/en/developers/docs/transactions/#types-of-transactions) currently supported on Ethereum, Arbitrum adds additional types listed below and [documented in full detail here](/docs/how-arbitrum-works/reference/geth#transaction-types). Many of these types are emitted when [bridging from a parent chain](/docs/arbitrum-essentials/bridging/cross-chain-messaging#ethereum-to-arbitrum-messaging).
...
| `104`                 | `ArbitrumRetryTxType`           | Used to [manually redeem a retryable ticket](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging) on a child chain that failed to execute automatically (usually due to low gas) |
| `105`                 | `ArbitrumSubmitRetryableTxType` | Used to [submit a retryable ticket](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#submission) via the Arbitrum bridge on the parent chain                                  |
...
| `ConditionalOptions` payload        | Description                                      |
| ----------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `knownAccounts`                     | map of address → (storageRootHash                | { `slot: expectedValue` }). The transaction is rejected unless each account's storage root matches, or each listed slot still holds the expected value. |
| `blockNumberMin` / `blockNumberMax` | L1 block-number window the transaction is valid. |
| `timestampMin` / `timestampMax`     | L2 timestamp window.                             |
...
| Field name | Description                                                                                                                                                                                       |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `from`     | On parent to child chain transactions, this field will contain the [_aliased_ version](/docs/how-arbitrum-works/deep-dives/l1-to-l2-messaging#address-aliasing) of the parent chain's `msg.sender` |
...
| Field name      | Description                                                                                                                                                                                                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `l1BlockNumber` | The block number of the first non-Arbitrum ancestor chain that is usable for `block.number` calls. More information in [Block numbers and time](/docs/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time) |
| `gasUsedForL1`  | The amount of gas spent on parent chain calldata in units of child chain gas. More information in [Gas and fees](/docs/how-arbitrum-works/deep-dives/gas-and-fees)                                                     |
...
| Field name      | Description                                                                                                                                                                                                                          |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `l1BlockNumber` | An approximate block number of the first non-Arbitrum ancestor chain that occurred before this child chain block. More information in [Block numbers and time](/docs/arbitrum-essentials/arbitrum-vs-ethereum/block-numbers-and-time) |
...
| `gasLimit`   | Value is fixed at `0x4000000000000`, but it's important to note that Arbitrum One currently has a 32M gas limit per block. See [Chain params](/docs/arbitrum-essentials/reference/chain-params) for the gas limit of other chains |
...
<VanillaAdmonition type="info" title="Note that the exact output for the `eth_syncing` RPC call of an out-of-sync Nitro node is not considered a stable API. It is still being actively developed and can be modified without notice between versions.">
...
<VanillaAdmonition type="info" title="Potential, but not expected error">
...
<VanillaAdmonition type="info" title="The `cargo-stylus` command-line tool uses the `stylusTracer` to replay transactions locally inside a debugger.">
More information can be found on [How to debug Stylus transactions using Cargo Stylus Replay](/docs/stylus/cli-tools/debugging-tx).
