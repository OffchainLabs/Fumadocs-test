...
This guide will help your team implement their own Data Availability (DA) provider that integrates with <Term id="arbitrum-nitro">Arbitrum Nitro</Term> using the DA API.
...
- **Fraud-proof compatible**: Full support for <Term id="bold">BoLD</Term> <Term id="challenge">challenge</Term> protocol and fraud proofs
...
1. **Reader RPC methods**: Recovers <Term id="batch">batch</Term> data from certificates and collects preimages for fraud proofs
2. **Writer RPC method**: Stores batch data and generates certificates
3. **<Term id="validator">Validator</Term> RPC methods**: Generates proofs for <Term id="fraud-proof">Fraud proof</Term> validation
...
- **Normal execution**: <Term id="batch-poster">Batch Poster</Term> stores data -> generates certificate -> posts to <Term id="layer-1-l1">L1</Term>
...
**Prerequisites**: Familiarity with JSON-RPC, Solidity, Ethereum L1/<Term id="layer-2-l2">L2</Term> architecture, and Arbitrum Nitro basics. Experience with any backend programming language.
...
Returns the header byte that identifies your DA provider in <Term id="sequencer">Sequencer</Term> messages.
...
4. **Verify data integrity**:
   ...
   - Check that the data matches the <Term id="commitment">commitment</Term> in the certificate
...
2. **Generate a certificate**:
   ...
   - Should include proof of authenticity (signature, <Term id="bls-signature">BLS Signature</Term>, etc.)
...
Certificates **always** come from L1 <Term id="sequencer-inbox">Sequencer Inbox</Term> messages\*\*, never from external DA providers. This process ensures:
...
**BoLD <Term id="challenge-protocol">challenge protocol</Term> Tests** (file: `system_tests/bold_challenge_protocol_test.go`):
...
The current default <Term id="client">client</Term> implementation relies on underlying transport encryption (e.g., TLS/JWT) for security. The signature parameter gets used for basic data integrity checking, not cryptographic authentication.
