...
After this step, you should visit [Customize ArbOS version](./customize-arbos) to see if your changes need to upgrade the ArbOS version. If so, please continue
...
<VanillaAdmonition type="info" title="The instructions provided in [How to run a full node](/docs/run-a-node/run-full-node) **will not** work with your Arbitrum chain node. See [Optional parameters (Arbitrum chain)](/docs/run-a-node/run-full-node#optional-parameters) for Arbitrum chain-specific CLI flags.">
...
- The <Term id="wasm-module-root">WASM module root</Term> (stored in `module-root.txt`) is a 32 byte hash usually expressed in hexadecimal which is a merkelization of `machine.wavm.br`.
