...
1. When you want to make changes to your Nitro code that affect the State Transition Function, or STF (you may refer to the [Customize STF docs](./customize-stf#introduction)), and
...
After you add `sayHi()` to `ArbSys.go` according to the guide in [customize precompile Option 1](./customize-precompile#option-1-add-new-methods-to-an-existing-precompile),
you need continue to modify [precompile.go](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/<Var name="nitroVersionTag" />/<Var name="nitroPathToPrecompiles" />/precompile.go).
...
After you add a new precompile named `ArbHi` according to the guide in [customize precompile Option 2](./customize-precompile#option-2-create-a-new-precompile) and make changes to [`precompile.go`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/<Var name="nitroVersionTag" />/<Var name="nitroPathToPrecompiles" />/precompile.go),
...
After you add a new state `myNumber` according to the guide in [customize precompile Option 5](./customize-precompile#option-5-call-and-modify-state), you also need to rewrite [`UpgradeArbosVersion`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/v3.1.0/arbos/<Var name="nitroPathToArbosState" />/arbosstate.go#L253) in [`arbosstate.go`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/v3.1.0/<Var name="nitroPathToArbosState" />/arbosstate.go):
...
<VanillaAdmonition type="warning" title="It should be noted that when you initialize the state (initial code is in [customize precompile Option 5](./customize-precompile#option-5-call-and-modify-state)), you need to initialize it to `0` or `null` value first to avoid a potential re-org on your chain.">
...
To prevent this, if you are using an external call to the precompile contract to change the value, you can refer to [point 1](./customize-arbos#1-add-a-new-method-to-existing-precompile-on-a-specific-arbos-version) or
[point 2](./customize-arbos#2-create-a-new-precompile-contract-on-a-specific-arbos-version) to set the activation time of the precompile contract method.
If your nitro code needs to call this method to change the state, you can continue reading [Step 4](./customize-arbos#4-any-changes-in-the-stf-logic-that-will-affect-the-final-execution-result-arbos-version-control).
...
In the above code, we use precompiles as an example. Some logic might also affect the STF, such as the methods in [`block_process.go`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/<Var name="nitroVersionTag" />/<Var name="nitroPathToArbos" />/block_processor.go), [`internal_tx.go`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/<Var name="nitroVersionTag" />/<Var name="nitroPathToArbos" />/internal_tx.go), [`tx_processor.go`](https://github.com/OffchainLabs/<Var name="nitroRepositorySlug" />/blob/<Var name="nitroVersionTag" />/<Var name="nitroPathToArbos" />/tx_processor.go) and so on.
...
Please continue reading [Customize STF](./customize-stf#step-3-run-the-node-without-fraud-proofs) for follow-up operations.

## Schedule ArbOS upgrade

After you add ArbOS version control to the Nitro code, you can update ArbOS. You can refer to the [ArbOS upgrade](/docs/launch-arbitrum-chain/operate/arbos-upgrade) document to upgrade. It is recommended that teams who opt to version control their custom ArbOS version choose an ArbOS version number that builds on top of the canonical releases shipped by Offchain Labs. As an example, if your team is customizing ArbOS 31, we recommend versioning your ArbOS version as ArbOS 32.
