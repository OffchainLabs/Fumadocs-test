...
This guide explains how to configure your <Term id="arbitrum">Arbitrum</Term> node to post [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) blob transactions to the <Term id="parent-chain">parent chain</Term>, which can significantly reduce data availability costs.
...
- Your <Term id="arbitrum-chain">Arbitrum chain</Term> must be running in **Rollup mode**
...
Your <Term id="arbos">ArbOS</Term> version must be **20 or higher**. To check your current version:

#### Method 1: Smart contract call

Call the `arbOSVersion()` function on the ArbSys <Term id="precompile">precompile</Term> contract:

- Contract address: `0x0000000000000000000000000000000000000064`
- Function: `arbOSVersion()` returns `uint256`
- You can call this using any Ethereum <Term id="client">client</Term> or block explorer on your Arbitrum chain
...
To enable blob <Term id="transaction">transaction</Term> posting, add the following configuration to your <Term id="batch-poster">Batch Poster</Term> node:
